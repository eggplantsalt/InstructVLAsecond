...
def main():
    ...
    correct_dir = image_root / "correct_image_text"
    correct_paths = []
    ...
    for i, text in enumerate(wrong_texts):
        cname = f"wrong_image_text_{i:02d}"
    ...
    for i, text in enumerate(random_texts):
        cname = f"random_image_text_{i:02d}"
        cdir = image_root / cname
        paths = []
        for t in range(num_frames):
            p = cdir / f"frame_{t:03d}.png"
    ...
    print("sample correct image:", conditions["correct_image_text"]["image_paths"][0])
    ...
    correct_mean = summaries["correct_image_text"]["mean_normalized_l2_6d"]
    prompt_mean = summaries["prompt_only"]["mean_normalized_l2_6d"]

    for cname, s in summaries.items():
        s["delta_vs_correct_mean_norm_l2_6d"] = float(s["mean_normalized_l2_6d"] - correct_mean)
        s["delta_vs_prompt_only_mean_norm_l2_6d"] = float(s["mean_normalized_l2_6d"] - prompt_mean)
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "delta_vs_prompt_only_mean_norm_l2_6d",
            ],
        )
        writer.writeheader()
        for cname, s in summaries.items():
            writer.writerow({
                ...
                "delta_vs_prompt_only_mean_norm_l2_6d": s["delta_vs_prompt_only_mean_norm_l2_6d"],
            })
    ...
    for cname, s in summaries.items():
        print(
            ...
            "delta_vs_prompt=", round(s["delta_vs_prompt_only_mean_norm_l2_6d"], 4),
