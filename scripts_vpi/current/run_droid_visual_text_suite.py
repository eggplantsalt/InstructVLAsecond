    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        for cname, s in summaries.items():
            writer.writerow({
                ...
                "external_instruction": s["external_instruction"],
                "prompt_mode": s["prompt_mode"],
...
def run_condition_group(
    ...
    for cname, cfg in conditions.items():
        ...
        np.save(group_dir / f"{cname}_pred_chunks_bridge_unnorm.npy", cond_bridge_chunks)
        ...
        print(
            ...
            "mean_bridge_l2_6d=", round(summary["mean_bridge_l2_6d"], 4),
...
def main():
    ...
    for t in range(num_frames):
        ...
        clean_bridge_chunks.append(actions)
        ...
        print(
            ...
            "bridge_first[:6]=", np.round(actions[0, :6], 4),
        )

    clean_bridge = np.stack(clean_bridge_chunks, axis=0)
    clean_norm = np.stack(clean_norm_chunks, axis=0)

    clean_dir = out_dir / "clean_baseline"
    clean_dir.mkdir(parents=True, exist_ok=True)
    np.save(clean_dir / "cleanImage_correctLang_defaultPrompt_pred_chunks_bridge_unnorm.npy", clean_bridge)
    ...
    clean_summary = {
        ...
        "pred_chunks_bridge_shape": list(clean_bridge.shape),
    ...
    for cname, s in main_summaries.items():
        print(
            ...
            "mean_bridge=", round(s["mean_bridge_l2_6d"], 4),
